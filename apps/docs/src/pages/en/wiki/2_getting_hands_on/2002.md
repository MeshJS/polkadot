...
function App() {
  ...
  const getCount = useCallback(async () => {
    const { result, output } = await wallet.contracts.incrementor.query.getCount(wallet.account.address, {
  ...
  const increment = async () => {
    ...
    await wallet.contracts.incrementor.tx
      .increment({ value: 0, gasLimit: wallet.api.registry.createType('WeightV2', gasRequired) }, incrementBy)
      .signAndSend(wallet.account.address, { signer: wallet.signer }, (result) => {
